...
public class Table {
    ...
    // IT ALSO SHOULD WORK WITH THREADS AND HAVE LOADING THING! i.e. it has to have thread.sleep.
    // SO IT WORK IN IT:S ON THREAD.
    public void select(String quert) {
        String[] separateInput = quert.split(" ");
        String[] columnsForView = separateInput[1].split(",");

        String[][] tableView = new String[columnsForView.length][getColumns().size()+1];
        for(String s:columnsForView){
            for(int j=0;j<)
        }
